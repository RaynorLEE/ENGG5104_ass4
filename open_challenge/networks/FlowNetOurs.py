...
class FlowNetEncoderERM(nn.Module):
    def __init__(self, args, input_channels=12, batchNorm=True, div_flow=20):
        super(FlowNetEncoderERM, self).__init__()
...
class FlowNetOurs(nn.Module):
    def __init__(self, args, input_channels = 6, div_flow=20):
        ...
        self.corr = SpatialCorrelationSampler(kernel_size=1, patch_size=21, stride=1)
        ...
        self.conv3_1 = nn.Sequential(
            nn.Conv2d(473, 256, kernel_size=3, stride=1, padding=1)
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(),
